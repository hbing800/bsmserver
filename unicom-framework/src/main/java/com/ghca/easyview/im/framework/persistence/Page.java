...
public class Page<T> {
    ...
    public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize) {
        ...
        if (StringUtils.isNumeric(no)) {
            CookieUtil.setCookie(response, "page", no);
            this.setPage(Integer.parseInt(no));
        } else if (request.getParameter("repage") != null) {
            no = CookieUtil.getCookie(request, "page");
            if (StringUtils.isNumeric(no)) {
                this.setPage(Integer.parseInt(no));
            }
        }
        ...
        if (StringUtils.isNumeric(size)) {
            CookieUtil.setCookie(response, "rows", size);
...
//            no = CookieUtils.getCookie(request, "rows");
   ...
   public String toString() {
        ...
        sb.append("<input type=\"text\" value=\"" + page + "\" onkeypress=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
        sb.append(funcName + "(this.value," + pageSize + ",'" + funcParam + "');\" onclick=\"this.select();\"/> / ");
        sb.append("<input type=\"text\" value=\"" + pageSize + "\" onkeypress=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
        sb.append(funcName + "(" + page + ",this.value,'" + funcParam + "');\" onclick=\"this.select();\"/> 条，");
        sb.append("共 " + records + " 条" + (message != null ? message : "") + "</a>");
        sb.append("</li>\n");
