...
public class ExportExcelUtils {
            ...
            for (Object obj : objs) {
                ...
                if (fieldNames != null) {
                    for (int num = 0; num < fieldNames.length; num++) {
                        Object value = Reflection2Util.invokeGetterMethod(obj, fieldNames[num]);
     ...
     */
    @SuppressWarnings({"unchecked"})
    public static void exportCMDBExcel(ExportSetInfo setInfo) throws
            IOException, IllegalArgumentException, IllegalAccessException {
        init();
        Set<Map.Entry<String, List>> set = setInfo.getObjsMap().entrySet();
        String[] sheetNames = new String[setInfo.getObjsMap().size()];
        ...
        for (Map.Entry<String, List> entry : set) {
            ...
            for (Object obj : objs) {
                ...
                if (fieldNames != null) {
                    for (int num = 0; num < fieldNames.length; num++) {
                        Map<String,Object> obj1 = (Map)obj;
                        Object value = obj1.get(fieldNames[num]);
    ...
    private static void createTableTitleRow(ExportSetInfo setInfo,
        ...
        if (setInfo.getFieldNames().get(sheetNum).length != 1) {
...
//        snCell.setCellValue("序号");
        ...
        for (int num = 0, len = setInfo.getHeadNames().get(sheetNum).length; num < len; num++) {
            ...
            sheets[sheetNum].setColumnWidth(num, setInfo.getHeadNames().get(sheetNum)[num].getBytes().length * 2 * 256);
